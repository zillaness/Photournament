...
(function () {
  ...
  function renderSetup(unit) {
    ...
    var actions = el('div', { class: 'row' }, [
      ...
      // An empty pool has nothing to rank and nothing to dedupe; offering the
      // bracket then marches the user through two purposeless screens. The
      // folder closes out directly instead: zero finalists, on to the next.
      canPass ? el('button', {
        ...
        class: 'btn', dataset: { t: 'finish-empty' }, text: 'Finish this folder — nothing kept',
        title: 'Close this folder with zero finalists and move on',
        onclick: function () {
          editUnit(G.unitId, 'unit:finishEmpty', function (u) {
            u.winners = [];
            u.currentPass = null;
            u.phase = 'done';
          });
    ...
    if (!canPass) {
      wrap.appendChild(el('div', { class: 'notice notice-warn', dataset: { t: 'empty' },
        text: 'The pool is empty. Rescue something from the cut pile, or finish the folder with nothing kept.' }));
    }
  }
  ...
  function finishEarly() {
    ...
    // The standing set as DECISIONS: one face per bundled burst. When bursts
    // are standing, "keep everything" has a duplicate question inside it, and
    // the modal must offer to answer it rather than silently export every
    // frame of every burst.
    var p0 = unit.currentPass;
    var faces = p0
      ? p0.kept.concat(p0.order.slice(p0.index * p0.gridSize))
      : S.slotsFor(unit).map(function (sl) { return sl.face; });
    var dupExtras = standing.length - faces.length;

    /**
     * The shared fold: cut what was judged out, keep the standing, and route.
     * 'done' takes the standing whole; 'runoff' seats one face per burst as
     * the finalists and sends the unit to the best-of-duplicates round, where
     * each burst gets its decision (and extras can still be kept) before
     * export.
     */
    function commitStop(winnerIds, phase) {
    ...
    body.appendChild(el('p', { class: 'small dim', text:
      ...
      (dupExtras > 0
        ? ' ' + dupExtras + ' of the standing are extra frames inside duplicate groups \u2014 ' +
          '\u201cbest of duplicates\u201d decides those before export instead of keeping every frame.'
        : '') }));
    body.appendChild(el('div', { class: 'row', style: 'margin-top:16px' }, [
      el('span', { class: 'spacer' }),
      el('button', { class: 'btn btn-quiet', text: 'Keep culling',
        onclick: function () { PT.dom.$('#modal').close(); } }),
      dupExtras > 0 ? el('button', {
        class: 'btn', dataset: { t: 'finish-early-dupes' },
        text: 'Keep ' + standing.length + ' \u2014 best of duplicates first',
        title: 'Stop culling, then decide each burst before export',
        onclick: function () {
          PT.dom.$('#modal').close();
          commitStop(faces, 'runoff');
          releaseCells();
          goSafe('runoff', { unitId: G.unitId });
        }
      }) : null,
      el('button', {
 ...
 * v1.7 (2026-07-29): Two exits stop being dead ends. Stop early grows a third
 *   path when bursts are standing — keep everything but route through the
 *   best-of-duplicates round first, one face per burst seated as finalists,
