 ...
 *
 * Cost: (N-1) for the main bracket, then roughly ceil(log2 N) - 1 comparisons per
 * additional place, because a champion's victim list is exactly one photo per
 * round it played. Measured on a 32-photo field asking for a top 8: 31 + 32 = 63
 * comparisons, against an information-theoretic floor of 39 and 496 for the
 * all-pairs alternative.
 *
...
(function () {
  ...
  function startSub(E, entrants, tag, place) {
    if (!entrants.length) { closeSub(E, null); return; }
    if (entrants.length === 1) { closeSub(E, entrants[0]); return; }
    E.sub = {
      tag: tag,
      place: place,
  ...
  function advanceRound(E) {
    var sub = E.sub, round = sub.rounds[sub.ri], winners = [], next = [], i;
    for (i = 0; i < round.length; i++) if (round[i].winner) winners.push(round[i].winner);
    if (winners.length <= 1) { closeSub(E, winners[0] || null); return; }
  ...
  /**
   * A sub-bracket produced its winner. Award the place, roll the candidate
   * pool forward, and start the next repechage — see the header comment for
   * why the pool is "everyone this photo personally beat".
   */
  function closeSub(E, winnerId) {
    E.sub = null;
    if (winnerId) {
      E.placements.push(winnerId);
      var pool = E.candidates.filter(function (id) { return id !== winnerId; });
      (E.victims[winnerId] || []).forEach(function (id) {
        if (pool.indexOf(id) < 0 && E.placements.indexOf(id) < 0) pool.push(id);
      });
      E.candidates = pool;
    }
    E.place = E.placements.length + 1;
    if (E.placements.length >= E.target || !E.candidates.length) { E.done = true; return; }
    startSub(E, seedRepechage(E), 'rep' + E.place, E.place);
  }

  /**
   * Seed the next mini bracket by main-bracket depth, descending, so the two
   * candidates with the strongest résumés meet in its FINAL rather than its
   * first round. Sorting the ids before the shuffle, and using the shuffle
   * position as the last tiebreak, keeps this independent of ingest order and
   * identical on every rebuild.
   */
  function seedRepechage(E) {
    var ids = E.candidates.slice().sort();
    var shuffled = PT.session.seededShuffle(ids, (E.seed ^ (E.place * 2654435761)) >>> 0);
    var rank = {};
  ...
  /**
   * Clamping is PER PANE and never written back to the shared centre. Two
   * photos of different orientation overflow the viewport on different axes,
   * so clamping the shared value against both would pin it to dead centre and
   * kill panning entirely — measured, and it is why this is not a one-liner.
   * An axis that does not overflow simply centres; every axis that can pan
   * pans to the same normalised coordinate in both panes.
   */
  function clampCenter(g, cx, cy) {
    var halfX = (g.vpW / 2) / (g.natW * g.S);
    var halfY = (g.vpH / 2) / (g.natH * g.S);
    return [
      halfX >= 0.5 ? 0.5 : Math.min(1 - halfX, Math.max(halfX, cx)),
      halfY >= 0.5 ? 0.5 : Math.min(1 - halfY, Math.max(halfY, cy))
    ];
  }
 ...
 *   bracket resume and a tiny persisted record from one mechanism. Matchup
