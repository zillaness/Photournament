...
(function () {
  ...
  PT.router.register('export', {
    mount: function (root) {
      ...
      // Sources are runtime-only: the browser hands files to a PAGE, and they
      // die with it. A reloaded session can rename, label and take the contact
      // sheet, but writing originals — zip or folder — needs the folder handed
      // over again. Matching is by relative path, which is the photo record's
      // identity, and re-picking the same folder reproduces it exactly.
      var missing = plan(sets).filter(function (it) { return !PT.sources[it.id]; });
      if (missing.length) {
        var reconnectInput = el('input', {
          type: 'file', id: 'exp-reconnect-files', multiple: true, style: 'display:none'
        });
        reconnectInput.setAttribute('webkitdirectory', '');
        reconnectInput.addEventListener('change', function () {
          var byPath = Object.create(null);
          Object.keys(st0.photos).forEach(function (id) {
            byPath[st0.photos[id].path] = id;
          });
          var hits = 0;
          Array.prototype.forEach.call(reconnectInput.files, function (f) {
            var id = byPath[f.webkitRelativePath || f.name];
            if (id) { PT.sources[id] = f; hits++; }
          });
          var still = plan(sets).filter(function (it) { return !PT.sources[it.id]; }).length;
          PT.toast(hits
            ? 'Reconnected ' + hits + ' original' + (hits === 1 ? '' : 's') +
              (still ? ' \u2014 ' + still + ' finalist' + (still === 1 ? '' : 's') + ' still unmatched.' : '.')
            : 'Nothing matched \u2014 is that the same folder this session was read from?');
          PT.router.go('export');
        });
        root.appendChild(el('div', { class: 'notice notice-warn', dataset: { t: 'reconnect' } }, [
          el('span', { text:
            missing.length + ' of ' + plan(sets).length + ' finalists\u2019 original files are not ' +
            'reachable in this session \u2014 the page was reloaded since the folder was read. ' +
            'Renaming, labels and the contact sheet still work; the zip and folder export need the ' +
            'originals back. Nothing is re-processed.' }),
          reconnectInput,
          el('button', {
            class: 'btn btn-sm', id: 'exp-reconnect', text: 'Reconnect the folder\u2026',
            onclick: function () { reconnectInput.click(); }
          })
        ]));
      }
    ...
    sets.forEach(function (set) {
      set.ids.forEach(function (id, i) {
        ...
        out.push({
          ...
          // plan() now also runs at mount, before the label store initialises.
  ...
  function doDownloadZip(sets) {
    ...
    buildZip(items, function (n, total) { status('Packing ' + n + ' / ' + total + '…'); })
      .then(function (out) {
        // A zip that silently packed nothing is worse than an error: it used
        // to download as a valid, EMPTY archive announcing the planned count.
        // Sources die with the page, so this is exactly what every reloaded
        // session got.
        if (out.packed === 0) {
          status('');
          PT.toast('No originals are reachable in this session \u2014 reconnect the folder above, then try again.');
          return;
        }
        var s = PT.store.get();
        var stamp = new Date().toISOString().slice(0, 10);
        saveBlob(out.blob, slugLabel(s.session.rootName || 'photournament') + '_finalists_' + stamp + '.zip');
        status(out.packed === items.length
          ? items.length + ' files in one zip, ' + PT.fmt.bytes(out.blob.size) + '.'
          : out.packed + ' of ' + items.length + ' files in the zip (' + PT.fmt.bytes(out.blob.size) +
            ') \u2014 ' + out.skipped.length + ' original' + (out.skipped.length === 1 ? '' : 's') +
            ' could not be read: ' + out.skipped.slice(0, 3).join(', ') +
            (out.skipped.length > 3 ? '\u2026' : ''));
        if (out.packed !== items.length) {
          PT.toast('The zip is missing ' + out.skipped.length + ' file' +
            (out.skipped.length === 1 ? '' : 's') + ' \u2014 reconnect the folder above and rebuild it.');
        }
      })
      .catch(function (e) {
        PT.warn('export', e);
 ...
 * v1.6 (2026-07-29): The zip stops lying. buildZip counts what actually packed
 *   and names what did not; an all-skipped build refuses to download instead of
 *   shipping a valid empty archive announcing the planned count \u2014 which is
 *   what every reloaded session got, because file sources die with the page.
 *   And the export screen now says so up front: a reconnect notice re-attaches
 *   the originals by relative path when any finalist's file is unreachable.
*/
